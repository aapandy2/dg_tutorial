...
%&= \partial_t \sum_{i=0}^{M} a_i(t) \int_k \psi_j \psi_i \, dx + \big[ f(\tilde{q}) \phi_j \big]^{x_R}_{x_L} - c \sum_{i=0}^{M} a_i(t) \int_k \psi_i \partial_x \psi_j \, dx \\
%&\equiv \bm{M} \cdot \partial_t \bm{a} + \bm{f} - c \bm{a} \cdot \bm{K} \\
%\implies \partial_t \bm{a} &= \bm{M}^{-1} \cdot \Big( c \bm{a} \cdot \bm{K} - \bm{f} \Big)
\end{aligned}
\end{equation}
